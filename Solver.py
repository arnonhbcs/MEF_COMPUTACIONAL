...
class Solver:
    """
    Essa classe Resolve uma estrutura de elementos finitos lineares, sujeita a cargas externas constantes,
    ...
    def __init__(self, trelica):
        ...
        self.trelica.computeKMatrix()
        self.trelica.computeFqMatrix()
        self.trelica.computeFqMatrix()
    ...
    def calcularQeF(self):
        ...
        simbolos = []
        for i in range(self.size):
            q = sp.symbols(f'q{i + 1}')
            R = sp.symbols(f'R{i + 1}')
            simbolos.append(q)
            simbolos.append(R)
        ...
        for element in self.trelica.elements:
            aKey, bKey = element.A.key, element.B.key
            elementF = element.getFMatrix()

            self.F[2 * aKey - 2] += elementF[0]
            self.F[2 * aKey - 1] += elementF[1]

            self.F[2 * bKey - 2] += elementF[0]
            self.F[2 * bKey - 1] += elementF[1]
        ...
        dictF = {f'R{i + 1}': self.F[i] for i in range(len(self.F))}
        dictQ = {f'q{i + 1}': self.q[i] for i in range(len(self.q))}
    ...
    def calcularForcasInternas(self):
        ...
        forcasInternas = {f'F_{i + 1}': self.trelica.elements[i].Fint for i in
